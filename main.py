...
# SSL ইনসিকিউর ওয়ার্নিং বন্ধ করার জন্য
...
# প্যারালাল সেটিংস (স্পিড কন্ট্রোল)
CONCURRENT_ACCOUNTS = 5   # একসাথে কয়টি একাউন্ট রান হবে
BOTS_PER_ACCOUNT = 8      # প্রতিটি একাউন্টের ভেতরে কয়টি থ্রেড চলবে
BASE_LIMIT = 400          # প্রতি একাউন্টের রিওয়ার্ড টার্গেট

# অফিসিয়াল অ্যাড এবং কুকি ডাটা
...
class AtokParallelBot:
    ...
    def load_files(self):
        ...
        with open(ATOK_FILE_NAME, "r", encoding="utf-8") as f:
            for line in f:
                ...
                token = tk.group(1).strip() if tk else ""
                if email and token:
        ...
        safe_log(f"Loaded {len(self.account_list)} accounts and {len(self.proxy_list)} proxies.")
        return len(self.account_list) > 0

    def get_reward_data(self, uid):
        # ATOK সার্ভারের জন্য এনকোডেড ডাটা
        ts = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")
        data = {"key": f"{uid}{ts}", "type": "MISSION_AD", "missionType": "VIDEO"}
        ...
        while True:
            ...
            try:
                ...
                if res.status_code == 200:
                    ad_data = res.json()
                    ad = ad_data['ad_networks'][0]
                    
                    # ২. ইমপ্রেশন পাঠানো
                    session.get(ad['ad']['impression_urls'][0], headers=headers, proxies=proxy, timeout=10, verify=False)
                    
                    # ৩. ভিডিও দেখার ওয়েট (মানুষের মতো বিরতি)
                    time.sleep(random.randint(6, 8))
                    
                    # ৪. রিওয়ার্ড ক্লেইম করা
                    ts_now = str(int(time.time() * 1000))
                    custom_data = self.get_reward_data(uid)
                    claim_url = ad['video_reward_urls'][0].replace("@gw_rwd_userid@", uid).replace("@gw_tmstmp@", ts_now).replace("@gw_rwd_custom_data@", custom_data)
                    
                    claim_res = session.get(claim_url, headers=headers, proxies=proxy, timeout=10, verify=False)
                    
                    if claim_res.status_code == 200:
                        with state['lock']:
                            if state['success'] < state['limit']:
                                state['success'] += 1
                                # প্রতি ১০০ টি ক্লেইম পরপর লগ দেখাবে
                                if state['success'] % 100 == 0:
                                    safe_log(f"⚡ [Progress] {email}: {state['success']}/{state['limit']} rewards.")
                else:
                    time.sleep(5) # অ্যাড না পেলে বিরতি
            except Exception:
                time.sleep(2)
            
            # প্রতি সাইকেলের মাঝে ছোট বিরতি
    ...
    def process_account(self, acc):
        ...
        # টার্গেট রেন্ডমাইজেশন (১২%)
        var = max(1, int(BASE_LIMIT * 0.12))
        actual_limit = BASE_LIMIT + random.randint(-var, var)
        
        state = {'success': 0, 'limit': actual_limit, 'lock': threading.Lock()}
        
        safe_log(f"▶ Starting Account: {email} | Target: {actual_limit}")
        
        threads = []
        for i in range(BOTS_PER_ACCOUNT):
            t = threading.Thread(target=self.bot_worker, args=(uid, email, state), daemon=True)
            t.start()
            threads.append(t)
            time.sleep(0.3)

        # সব থ্রেড শেষ হওয়া পর্যন্ত ওয়েট করবে
        for t in threads:
            t.join()
            
        safe_log(f"✅ Completed Account: {email} | Total Rewards: {state['success']}")
    ...
    def run_forever(self):
        ...
        while True:
            ...
            # একাউন্ট লিস্ট এলোমেলো করা
            random.shuffle(self.account_list)
            
            # প্যারালাল প্রসেসিং শুরু
            with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENT_ACCOUNTS) as executor:
                # ম্যাপ ফাংশন দিয়ে সব একাউন্টকে প্যারালাল এক্সেকিউশনে পাঠানো
                executor.map(self.process_account, self.account_list)
            
            safe_log(f"\n🏆 CYCLE #{cycle} FINISHED. Waiting 2 minutes for next cycle...")
            time.sleep(120)
            cycle += 1

if __name__ == "__main__":
    # স্ক্রিন ক্লিয়ার (ঐচ্ছিক)
    os.system('cls' if os.name == 'nt' else 'clear')
    
    bot_manager = AtokParallelBot()
    if bot_manager.load_files():
        bot_manager.run_forever()
    else:
        safe_log("Bot could not start. Check your accounts.txt file.")
